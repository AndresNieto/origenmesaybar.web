...
import { DEFAULT_FOODS } from "../FoodsComponent/consts";
...
const FoodDetailsComponent: FunctionComponent<FoodDetailsComponentProps> = () => {
  ...
  return (
    <main className="max-w-screen-xl mx-auto px-6 my-16">
      ...
      <BackComponent />
      {foods?.filter(item => item.name === title)?.map((food) => (
        <div className="flex flex-col justify-center items-center h-screen">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-10" key={food.id}>
            ...
            <div className="order-2 md:order-1 lg:order-1 flex flex-col justify-center">
              <h1 className="text-center md:text-left lg:text-left text-3xl lg:text-4xl font-semibold poppins pb-4 text-gray-700 select-none">{food.name}</h1>
